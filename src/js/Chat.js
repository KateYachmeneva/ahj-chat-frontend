...
export default class Chat {
  ...
  init() {
    this.bindToDOM();
    this.registerEvents();
    this.modalForm = new ModalForm(this.container);
    this.modalForm.bindToDOM();
  ...
  registerEvents() {
    this.connectButton.addEventListener('click', () => this.modalForm.showModal());
  ...
  async onEnterChatHandler(newUser) {
    ...
    if (response.status === 'ok') {
      ...
      this.websocket = new WebSocket('wss://websock-chat.herokuapp.com//chat');
      this.websocket.addEventListener('message', (event) => this.renderMessage(event));
      window.addEventListener('beforeunload', () => this.websocket.send(
        JSON.stringify({
          type: 'exit',
          user: this.user,
        }),
      ...
      this.modalForm.showHint(response.message);
    }
  }
